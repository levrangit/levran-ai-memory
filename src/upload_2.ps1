...
function Start-RobocopyCopy {
    param([string]$Source,[string]$Destination)

    Write-Host "  Запуск robocopy..."
    Write-Host "  FROM: $Source"
    Write-Host "  TO:   $Destination"
    Write-Host "  Режим: /E /MT:16 /R:1 /W:1"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "  robocopy завершён. EXIT CODE: $rc"

    if ($rc -ge 8) { return $rc }
    return 0
}
...
foreach ($dbFile in $dbFiles) {
    ...
    # Robocopy копирует dump напрямую, без архива и без rclone.
    $rc = Start-RobocopyCopy -Source $dumpRoot -Destination $destination

    if ($rc -ne 0) {
        throw "robocopy завершился с кодом $rc для $id"
    }
...
Write-Host ""
Write-Host "UPLOAD завершён. Архивирование не выполнялось. Использовался robocopy."
exit 0
